        ...
        stage("Run tests") {
            steps {
                dir("pags_api_tests") {
                    ...
                    sh "docker run --net pags-net -e 'BASE_URL=http://app:8081/status/' -v /var/jenkins_home/workspace/prova-pags-automacao:/target/surefire-reports --name tests my_tests"
                }
                sh "ls"
            }
        }
