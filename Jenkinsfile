        ...
        stage("Run tests") {
            steps {
                ...
                dir("pags_api_tests") {
                    ...
                    sh "docker run --net pags-net -e 'BASE_URL=http://app:8081/status/' -v jenkins-data:/target/surefire-reports -v /var/run/docker.sock:/var/run/docker.sock --name tests my_tests"
                }
                dir ("../..") {
